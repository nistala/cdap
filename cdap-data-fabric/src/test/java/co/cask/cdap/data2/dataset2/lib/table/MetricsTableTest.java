...
public abstract class MetricsTableTest {
  ...
  public void testConcurrentIncrement() throws Exception {
    ...
    Collection<? extends Thread> threads =
        ImmutableList.of(new IncThread(table, A, inc1, rounds),
                         new IncThread(table, A, inc2, rounds),
                         new IncAndGetThread(table, A, R, 5, rounds),
                         new IncAndGetThread(table, A, R, 2, rounds));
    ...
    Assert.assertEquals(rounds, Bytes.toLong(table.get(A, X)));
    Assert.assertEquals(3 * rounds, Bytes.toLong(table.get(A, Y)));
    Assert.assertEquals(2 * rounds, Bytes.toLong(table.get(A, Z)));
    Assert.assertEquals(7 * rounds, Bytes.toLong(table.get(A, R)));
  }
