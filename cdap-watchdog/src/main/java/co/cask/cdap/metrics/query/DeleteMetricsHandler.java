...
public class DeleteMetricsHandler extends BaseMetricsHandler {
  ...
  private void deleteTableEntries(MetricsScope scope, String contextPrefix,
    ...
    if (contextPrefix == null && tag == null && metricPrefix == null) {
      for (int i : timeSeriesResolutions) {
        metricsTableCaches.get(scope).getUnchecked(i).clear();
      }
      aggTable.clear();
    } else if (tag == null) {
      for (int i : timeSeriesResolutions) {
        metricsTableCaches.get(scope).getUnchecked(i).delete(contextPrefix, metricPrefix);
      }
      ...
      for (int i : timeSeriesResolutions) {
        metricsTableCaches.get(scope).getUnchecked(i).delete(scanQuery);
      }
