...
abstract class AbstractSparkContext implements SparkContext {
   ...
   * @return updated {@link Configuration}
   * @throws IOException if the given {@link Stream} is not found or the {@link StreamEventDecoder} was not indentified
   */
  ...
  private void configureStreamInput(Configuration hConf, StreamBatchReadable stream, Class<?> vClass)
    ...
    if (decoderType == null) {
      ...
      setStreamEventDecoder(hConf, vClass);
    } else {
  ...
  private void setStreamEventDecoder(Configuration hConf, Class<?> vClass) throws IOException {
